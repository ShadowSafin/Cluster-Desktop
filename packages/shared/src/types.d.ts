...
export type MessageKind = 'chat' | 'plan' | 'summary' | 'error' | 'warning' | 'info' | 'tool-result';
...
export type ToolStatus = 'pending' | 'running' | 'success' | 'error' | 'cancelled' | 'rejected';
...
export type ToolArtifact = {
    ...
    type: 'file';
    path: string;
    action: 'read' | 'written' | 'created' | 'deleted';
} | {
    type: 'log';
    lines: string[];
} | {
...
export type PackageManager = 'npm' | 'pnpm' | 'yarn' | 'bun' | 'pip' | 'uv' | 'poetry' | 'cargo' | 'go';
...
export declare function createEmptySession(init: {
    id: string;
    projectRoot: string;
    model: string;
    title?: string;
