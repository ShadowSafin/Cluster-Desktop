...
export interface RepoIntelligence {
  ...
  fileGroups: Array<{ area: string; files: string[]; language?: string }>;
  testFiles: string[];
}
  ...
  try {
    ...
    for (const file of files) {
      for (const hint of FRAMEWORK_HINTS) {
        if (hint.pattern.test(file) && !found.includes(hint.framework)) found.push(hint.framework);
      }
    }
  } catch {
    // ignore
...
async function readGitDiff(root: string): Promise<{ branch: string | null; changed: string[]; summary: string }> {
  try {
...
async function discoverCommands(root: string): Promise<RepoIntelligence['commands']> {
  const commands: RepoIntelligence['commands'] = { build: [], test: [], lint: [], format: [] };
  try {
    const pkgRaw = await fs.readFile(path.join(root, 'package.json'), 'utf8');
    const pkg = JSON.parse(pkgRaw) as { scripts?: Record<string, string> };
    const scripts = pkg.scripts ?? {};
    const mapping: Record<string, keyof typeof commands> = {
      build: 'build',
      compile: 'build',
      test: 'test',
      'test:unit': 'test',
    ...
    for (const [script, kind] of Object.entries(mapping)) {
      if (scripts[script]) commands[kind].push(scripts[script]);
    }
    if (commands.build.length === 0 && scripts.build) commands.build.push(scripts.build);
    if (commands.test.length === 0 && scripts.test) commands.test.push(scripts.test);
  } catch {
    // no package.json
  }
  // Fallbacks per language
  if (commands.test.length === 0) commands.test.push('npm test');
  if (commands.build.length === 0) commands.build.push('npm run build');
  return commands;
}

async function groupFiles(root: string): Promise<Array<{ area: string; files: string[]; language?: string }>> {
  try {
...
export async function gatherRepoIntelligence(root: string): Promise<RepoIntelligence> {
  ...
  if (packageManager === 'npm' || packageManager === 'pnpm' || packageManager === 'yarn' || packageManager === 'bun') projectKind = 'node';
  else if (packageManager === 'cargo') projectKind = 'rust';
  else if (packageManager === 'go') projectKind = 'go';
  else if (packageManager === 'pip' || packageManager === 'poetry') projectKind = 'python';
