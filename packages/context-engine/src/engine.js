...
import { chunkFile, selectRelevantChunks } from './chunking.js';
import { extractSymbols } from './symbols.js';
import { gatherRepoIntelligence } from './repoIntelligence.js';
export class ContextEngine {
     ...
     */
    async selectContext(query, candidates) {
        ...
        const candidatesWithMeta = await Promise.all(fileList.slice(0, 80).map(async (file) => {
    ...
    buildSummary(query, ranked, repo, chunks, symbols) {
        const lines = [];
        ...
        if (repo) {
            ...
            if (repo.git?.recentChangedFiles.length)
                lines.push(`Recent changes: ${repo.git.diffSummary}`);
            lines.push(`Areas: ${repo.fileGroups.map((g) => `${g.area} (${g.files.length})`).join(', ')}`);
        }
        if (chunks.length > 0)
            lines.push(`Chunked ${chunks.length} large files, kept relevant slices`);
        if (symbols.length > 0)
            lines.push(`Symbols: ${symbols.slice(0, 6).map((s) => s.name).join(', ')}${symbols.length > 6 ? '…' : ''}`);
    ...
    async rank(query, files) {
        const meta = await Promise.all(files.map(async (file) => ({
            path: file,
