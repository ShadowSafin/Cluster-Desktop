...
function scorePath(path: string, tokens: string[]): { score: number; reasons: string[] } {
...
export function rankFiles(
  candidates: Array<{ path: string; size: number; language?: string; contentPreview?: string; area?: string }>,
  ...
  const scores: FileScore[] = candidates.map((file) => {
  ...
  for (const scored of scores) {
    ...
    if (scored.score <= 0 && selected.length >= 3) continue;
    const estTokens = Math.min(scored.size / 4, 4000);
    if (tokensUsed + estTokens > maxTokens && selected.length >= 5) break;
...
export function groupByArea(files: FileScore[]): Map<string, FileScore[]> {
