...
export function formatWorkspaceContext(info: WorkspaceInfo): string {
  const lines: string[] = [];
  ...
  if (info.manifestFiles.length > 0) lines.push(`- Manifests: ${info.manifestFiles.join(', ')}`);
  ...
  if (info.git) {
    ...
    lines.push(
      `- Working tree: ${g.dirty ? `dirty (${g.staged} staged, ${g.unstaged} modified, ${g.untracked} untracked)` : 'clean'}`,
    );
    if (g.lastCommit) lines.push(`- Last commit: ${g.lastCommit}`);
  } else {
  ...
  if (commandHints.length > 0) {
    ...
    for (const hint of commandHints) lines.push(`- ${hint}`);
  }
  ...
  if (info.project.scripts && Object.keys(info.project.scripts).length > 0) {
    ...
    for (const [name, command] of scripts) lines.push(`- ${name}: ${command}`);
  }
...
export function formatWorkspaceHeadline(info: WorkspaceInfo): string {
  const parts: string[] = [info.name];
  if (info.project.kind !== 'unknown') parts.push(info.project.kind);
  if (info.languages.length > 0) parts.push(info.languages.slice(0, 2).join('/'));
  return parts.join(' · ');
}
