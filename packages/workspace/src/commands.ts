...
const SCRIPT_CANDIDATES: Record<keyof InferredCommands, string[]> = {
...
function pick(scripts: Record<string, string>, candidates: string[]): string[] {
  const out: string[] = [];
 ...
 */
export function inferCommands(manifest: ManifestInfo): InferredCommands {
  ...
  switch (manifest.kind) {
    case 'node': {
      ...
      if (commands.build.length === 0 && manifest.scripts['build']) commands.build.push(manifest.scripts['build']);
      if (commands.test.length === 0 && manifest.scripts['test']) commands.test.push(manifest.scripts['test']);
      if (commands.build.length === 0) commands.build.push('npx tsc --noEmit');
      if (commands.test.length === 0) commands.test.push(`${run} test`);
      break;
    }
    case 'python':
      if (commands.test.length === 0) commands.test.push('pytest -q');
      if (commands.lint.length === 0) commands.lint.push('ruff check .');
      if (commands.format.length === 0) commands.format.push('ruff format .');
      break;
    case 'go':
...
export function describeCommands(commands: InferredCommands): string[] {
  const out: string[] = [];
  for (const [kind, list] of Object.entries(commands)) {
    for (const command of list.slice(0, 2)) out.push(`${kind}: ${command}`);
  }
  return out;
}
