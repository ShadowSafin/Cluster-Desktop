...
export async function readGitPorcelain(cwd) {
    if (!(await isGitRepository(cwd)))
        return null;
 ...
 * e.g. `main* +3 ~2 ?1`
 */
export function formatGitState(git) {
    if (!git)
        return 'no git';
    const parts = [git.dirty ? `${git.branch}*` : git.branch];
    if (git.staged > 0)
        parts.push(`+${git.staged}`);
    if (git.unstaged > 0)
        parts.push(`~${git.unstaged}`);
    if (git.untracked > 0)
        parts.push(`?${git.untracked}`);
    return parts.join(' ');
}
