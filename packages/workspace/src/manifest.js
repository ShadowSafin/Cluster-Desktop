...
const EMPTY = { kind: 'unknown', packageManager: null, name: null, scripts: {} };
 ...
 * read is best-effort.
 */
export async function readManifest(root, manifests) {
    if (manifests.includes('package.json')) {
        ...
        if (pkg) {
            ...
            const manager = declared ? declared.split('@')[0] : lockManager;
    ...
    if (manifests.includes('pyproject.toml')) {
        try {
            ...
            const manager = poetry ? 'poetry' : 'pip';
